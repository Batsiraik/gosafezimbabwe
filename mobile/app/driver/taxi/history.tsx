...
import { IconArrowBack, IconCar, IconLocation } from '@/components/DashboardIcons';
...
export default function DriverTaxiHistoryScreen() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const data = await driverTaxiApi.rideHistory(token);
      setRides((data.rides as RideItem[]) || []);
    } catch (_) {
      setRides([]);
    } finally {
      setLoading(false);
                ...
                <View key={ride.id} style={styles.card}>
                  <View style={styles.cardHeader}>
                    <View style={[styles.statusBadge, { backgroundColor: st.bg }]}>
                      <Text style={[styles.statusText, { color: st.text }]}>{formatStatus(ride.status)}</Text>
                    </View>
                    <Text style={styles.price}>
                      ${(ride.finalPrice ?? ride.driverBidPrice ?? ride.price ?? 0).toFixed(2)}
                    </Text>
                  </View>
                  ...
                  <View style={styles.addressRow}>
                    <IconLocation color="rgba(255,255,255,0.7)" size={14} />
                    <Text style={styles.addressMuted}>{ride.destinationAddress}</Text>
                  </View>
                  {ride.distance != null && (
                    <Text style={styles.meta}>Distance: {ride.distance.toFixed(1)} km</Text>
                  )}
                  {ride.user?.fullName && (
                    <Text style={styles.meta}>Passenger: {ride.user.fullName}</Text>
                  )}
                </View>
              );
            })
