...
import { ArrowLeft, Car, MapPin, DollarSign, Calendar, Clock, CheckCircle, XCircle, Loader2, Filter } from 'lucide-react';
...
export default function TaxiDriverHistoryPage() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center">
              <button
                onClick={() => router.push('/driver/taxi/dashboard')}
                className="mr-4 p-2 rounded-full bg-white/20 hover:bg-white/30 transition-colors"
              >
                <ArrowLeft className="w-5 h-5 text-white" />
              </button>
              <h1 className="text-2xl font-bold text-white">Ride History & Earnings</h1>
            </div>
          </div>
        </div>
      </div>
        ...
        >
          ...
          {stats && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-4 shadow-xl border border-white/20">
                <p className="text-white/70 text-xs mb-1">Total Rides</p>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 shadow-xl border border-white/20">
            <h2 className="text-white font-semibold text-lg mb-4">Rides</h2>
            
            {filteredRides.length === 0 ? (
              <div className="text-center py-8">
                <Car className="w-12 h-12 text-white/30 mx-auto mb-3" />
                <p className="text-white/70">No rides found</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right ml-4">
                        ...
                        <p className="text-nexryde-yellow font-bold text-lg mt-2">
                          ${(ride.driverBidPrice || ride.price).toFixed(2)}
                        </p>
                        {ride.driverBidPrice && ride.driverBidPrice !== ride.price && (
